...
// get the state of the light
    getStateLight1() {
      if (this.stateLight1 == null) {  // first Call
        this.svc.getLightState(1).subscribe(res => {
          ...
          if ((this.prevStateLight1.on !== res.state.on) && ((this.prevStateLight1.bri !== res.state.bri))) {
            this.stateLight1 = res;
            this.stateSummaryLight1.on = res.state.on;
            this.stateSummaryLight1.brightness = res.state.bri;
            this.prevStateLight1 = res.state;
 ...
 // change to on/off switch but NOT brightness: PRINT on/off
          if ((this.prevStateLight1.on !== res.state.on) && ((this.prevStateLight1.bri==res.state.bri))) {
            this.stateLight1 = res;
            this.stateSummaryLight1.on = res.state.on;
            this.stateSummaryLight1.brightness = res.state.bri;
...
//  change to brightness but NOT on/off switch: PRINT brightness
          if ((this.prevStateLight1.on == res.state.on) && ((this.prevStateLight1.bri !== res.state.bri))){
            this.stateLight1 = res;
            this.stateSummaryLight1.on = res.state.on;
            this.stateSummaryLight1.brightness = res.state.bri;
      ...
      getStateLight2() {
        // first Call
            if (this.stateLight2 == null) {
              ...
              this.svc.getLightState(2).subscribe(res => {
                // debugger
// change to on/off switch and brightness: PRINT BOTH

                if ((this.prevStateLight2.on !== res.state.on) && ((this.prevStateLight2.bri !== res.state.bri)))
                {
                  this.stateLight1 = res;
                  this.stateSummaryLight2.on = res.state.on;
                  this.stateSummaryLight2.brightness = res.state.bri;
